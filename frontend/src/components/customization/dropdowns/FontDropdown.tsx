...
import { ChevronDown, Type, Lock, Crown } from 'lucide-react';
...
const fonts = [
    { id: 'poppins', name: 'Poppins', sample: 'Clean & Modern' },
    { id: 'chillax', name: 'Chillax', sample: 'Smooth & Stylish' },
...
export default function FontDropdown({ profile, onFontChange, isPremium }: FontDropdownProps) {
    const { user: contextUser } = useUser();

    const [isOpen, setIsOpen] = useState(false);
    const dropdownRef = useRef<HTMLDivElement>(null);
    const currentFont = profile.text_font || 'poppins';
    ...
    const handleFontSelection = (font: string) => {
        if (!isPremium && fonts.find(f => f.id === font)?.premium) {
            return;
        }
        onFontChange(font);
        setIsOpen(false);
        ...
        <div className="relative" ref={dropdownRef}>
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm ${getFontClass(currentFont)} text-white`}>
                        {currentFontData?.name || 'Poppins'}
                    </span>
                </div>
                ...
                <div className="absolute top-full left-0 w-full mt-1.5 z-20 bg-black border border-zinc-800/50 rounded-lg 
                    ...
                    <div className="max-h-64 overflow-y-auto py-1.5 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent">
                        {fonts.map((font) => {
                            ...
                            const isPremiumLocked = font.premium && !isPremium;

                            return (
                                <button
                                    key={font.id}
                                    onClick={() => !isPremiumLocked && handleFontSelection(font.id)}
                                ...
                                >
                                    <div className="flex flex-col space-y-1 w-full">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-sm ${getFontClass(font.id)} text-white`}>
                                                {font.name}
                                            </span>
                                        </div>
                                        <span className="text-xs text-zinc-500 text-left">{font.sample}</span>
                                    </div>
                    ...
                    {!isPremium && (
                        <Link href="/pricing" className="group block mt-1 px-3 py-3 border-t border-zinc-800/50 bg-gradient-to-r from-purple-900/20 to-purple-800/10 hover:from-purple-900/30 hover:to-purple-800/20 transition-all duration-300">
                            <div className="flex items-center gap-2 text-xs">
                                ...
                                <span className="text-purple-200">Unlock premium fonts with Premium</span>
